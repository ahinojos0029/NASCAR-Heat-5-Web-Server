...
def build_game_response(gid, g, viewer_session=None):
    ...
    session_index = players.index(viewer_session) if viewer_session in players else 0
    return {
        "id": gid,
        "s": {
...
async def parse_body(request: Request) -> dict:
    """Safely parse JSON body; return {} on empty or invalid."""
    try:
...
async def get_user(user_id: str):
    ...
    u = users[user_id]
    return resp({"id": user_id, "name": u.get("name", "player")})
...
# ---------------- GAME ----------------
@app.post("/game")
async def create_game(request: Request, mgi_bearer_token: str = Header(None)):
